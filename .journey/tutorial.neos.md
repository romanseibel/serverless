...
Google [Application Default Credentials (ADC)](https://cloud.google.com/docs/authentication/provide-credentials-adc) to find credentials. In this case it was using the credentials of the
...
[error budget](https://cloud.google.com/blog/products/management-tools/sre-error-budgets-and-maintenance-windows)
 (see also [Emracing Risk](https://sre.google/sre-book/embracing-risk/)) and 
 quickly rollback to a safe state.
...
Google Search, Ads, Gmail, Android, YouTube, many Cloud Services, to name just a few
...
We also have a direct measurement of a service’s behavior: the proportion of
...
Finally, also create an alert on Error Budget [Burn Rate](https://sre.google/workbook/alerting-on-slos/) 
that will notify you in case a [fast-burn](https://cloud.google.com/stackdriver/docs/solutions/slo-monitoring/alerting-on-budget-burn-rate) 
of your error budget gets detected. Create an new alert, that:
...
confidence in your new revision you can gradually increase traffic until you reach 100% of
...
service has dropped to around 90%, which is below your SLO target of 99%!
